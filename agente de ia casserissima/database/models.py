...
Modelos de base de datos para el sistema de pastelería, migrados del sistema base
y expandidos para soportar el Agente IA.
"""
import json
...
# ── NUEVAS TABLAS PARA EL AGENTE IA ───────────────────────────────────────────

class ScheduledJob(Base):
    """
    Trabajos programados persistentes para proactividad dinámica.
    """
    __tablename__ = "scheduled_jobs"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    chat_id: Mapped[str] = mapped_column(String(50), nullable=False)
    job_type: Mapped[str] = mapped_column(String(50), nullable=False) # e.g. "inventory_check"
    description: Mapped[str] = mapped_column(Text, nullable=False)
    cron_expression: Mapped[str] = mapped_column(String(50), nullable=False)
    parameters: Mapped[str] = mapped_column(Text, nullable=False, default="{}") # JSON string
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime, default=lambda: datetime.now(timezone.utc)
    )
...
class AgentLog(Base):
    """
    Registro de interacciones del agente IA para observabilidad.
    """
    ...
    tools_used: Mapped[str] = mapped_column(Text, nullable=False, default="[]") # JSON string
